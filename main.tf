// AWS CERDENTIALS PROVIDER RESOLVE THE CONNECTION
provider "aws" {
  region                  = "us-east-1"
  shared_credentials_file = "/Users/sailesh/.aws/creds"
...
module "cluster" {
  ...
  security_groups = [
  distinct(concat(list(var.sg_list), compact(var.security_groups)))]
...
module "consul" {
  ...
  cluster_sg_id = var.security_groups
